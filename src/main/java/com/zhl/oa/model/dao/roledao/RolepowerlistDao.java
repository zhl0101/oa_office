package com.zhl.oa.model.dao.roledao;
...
public interface RolepowerlistDao extends JpaRepository<Rolepowerlist, Long>{
	//找所有的父菜单
	@Query("select new com.zhl.oa.model.entity.role.Rolemenu(menu.menuId,menu.menuName,menu.menuUrl,menu.show,role.check,menu.parentId,menu.menuIcon,menu.sortId,menu.menuGrade) "
	...
	//找所有的子菜单
	@Query(value = "select new com.zhl.oa.model.entity.role.Rolemenu(menu.menuId,menu.menuName,menu.menuUrl,menu.show,role.check,menu.parentId,menu.menuIcon,menu.sortId,menu.menuGrade) "
			+ "from Rolepowerlist as role,SystemMenu as menu where role.menuId.menuId=menu.menuId "
			+ "and menu.parentId!=?1 and role.roleId.roleId=?2 order by menu.sortId")
